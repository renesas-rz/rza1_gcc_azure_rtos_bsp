...
static NX_DHCP	NO_CACHE_SECTION dhcp_0 __attribute__ ((aligned (32)));
...
static ULONG NO_CACHE_SECTION sample_pool_stack[SAMPLE_POOL_SIZE / sizeof(ULONG) +  32/sizeof(ULONG)] __attribute__ ((aligned (32)));
...
UINT  status;
    ...
    status = nx_packet_pool_create(&pool_0, "NetX Main Packet Pool", SAMPLE_PACKET_SIZE,
                                    (ULONG*)(((int)sample_pool_stack + 31) & ~31), sample_pool_stack_size);
