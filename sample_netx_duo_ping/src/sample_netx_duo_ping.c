/* Simple ping demonstration for Renesas RZ R7S72100 CPU Board.  Simply execute this
   demonstration and ping to the board from another device. Note that the IP address
   is setup in the nx_ip_create call below.  */
...
/* Don't cache! */
NX_DHCP	NO_CACHE_SECTION	dhcp_client __attribute__ ((aligned (16)));
#endif
...
#define NX_PACKET_POOL_SIZE     ((NX_PACKET_PAYLOAD_SIZE + sizeof(NX_PACKET)) * 36)

/* Add 32 to allow 32-byte alignment of rx buffers. */
/* Divide because this is a ULONG (not byte) array. */
/* Don't cache packet pool! */
ULONG	NO_CACHE_SECTION	packet_pool_area[NX_PACKET_POOL_SIZE/sizeof(ULONG) + 32/sizeof(ULONG)] __attribute__ ((aligned (16)));
