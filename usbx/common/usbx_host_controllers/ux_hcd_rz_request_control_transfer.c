...
/*  FUNCTION                                               RELEASE        */ 
/*                                                                        */ 
/*    _ux_hcd_rz_request_control_transfer                 PORTABLE C      */ 
/*                                                           5.8          */ 
/*  AUTHOR                                                                */ 
/*                                                                        */ 
/*    Thierry Giron                                                       */ 
...
/*     setting the RZ endpoint as a candidate for transfer.               */ 
/*                                                                        */ 
/*  INPUT                                                                 */ 
/*                                                                        */ 
...
ULONG               max_packet_size;
    ...
    *(setup_request + UX_SETUP_REQUEST) =       transfer_request -> ux_transfer_request_function;
    ...
    setup_td =  ed -> ux_rz_ed_tail_td;
    setup_td -> ux_rz_td_buffer =  setup_request;
    setup_td -> ux_rz_td_length =  UX_SETUP_SIZE;
    chain_td =  setup_td;

    /* Attach the endpoint and transfer_request to the TD.  */
    setup_td -> ux_rz_td_transfer_request =  transfer_request;
    setup_td -> ux_rz_td_ed =  ed;
    
    hcd_rz ->  ux_hcd_rz_number_tds++ ;

    /* Setup is OUT.  */
    setup_td -> ux_rz_td_direction =  UX_RZ_TD_OUT;

    /* Mark TD toggle as being DATA0.  */
    setup_td -> ux_rz_td_toggle =  0;

    /* Mark the TD with the SETUP phase.  */
    setup_td -> ux_rz_td_status |=  UX_RZ_TD_SETUP_PHASE;
    ...
    while (transfer_request_payload_length != 0)
    {
        ...
        data_td -> ux_rz_td_length =  control_packet_payload_length;
        ...
        /* The Toggle value.  */
        data_td -> ux_rz_td_toggle =  toggle;
        ...
        chain_td -> ux_rz_td_next_td =  data_td;
        chain_td -> ux_rz_td_next_td_transfer_request =  data_td;
        chain_td =  data_td;
        hcd_rz ->  ux_hcd_rz_number_tds++ ;
    ...
    status_td =  _ux_hcd_rz_regular_td_obtain(hcd_rz);
    ...
    /* Attach the endpoint and transfer request to the TD.  */
    status_td -> ux_rz_td_transfer_request =  transfer_request;
    status_td -> ux_rz_td_ed =  ed;

    /* Mark the TD with the STATUS phase.  */
    status_td -> ux_rz_td_status |=  UX_RZ_TD_STATUS_PHASE;

    /* The direction of the status phase is IN if data phase is OUT and
       vice versa.  */
    if ((transfer_request -> ux_transfer_request_type&UX_REQUEST_DIRECTION) == UX_REQUEST_IN)

        status_td -> ux_rz_td_direction =  UX_RZ_TD_OUT;
    else

        status_td -> ux_rz_td_direction =  UX_RZ_TD_IN;

    /* No data payload for the status phase.  */
    status_td -> ux_rz_td_buffer =  0;
    status_td -> ux_rz_td_length =  0;

    /* Status Phase toggle is ALWAYS 1.  */
    status_td -> ux_rz_td_toggle =  1;

    /* Hook the status phase to the previous TD.  */
    chain_td -> ux_rz_td_next_td =  status_td;
    ...
    if (tail_td == UX_NULL)
    {
        ...
        if (data_td != UX_NULL)
            data_td -> ux_rz_td_status =  UX_UNUSED;
        status_td -> ux_rz_td_status =  UX_UNUSED;
        return(UX_NO_TD_AVAILABLE);
    }

    /* Hook the new TD to the status TD.  */
    status_td -> ux_rz_td_next_td =  tail_td;
   ...
   /* Wait for the completion of the transfer request.  */
    status =  _ux_utility_semaphore_get(&transfer_request -> ux_transfer_request_semaphore, UX_CONTROL_TRANSFER_TIMEOUT);
