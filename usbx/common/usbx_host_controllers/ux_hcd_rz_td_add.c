...
UINT  _ux_hcd_rz_td_add(UX_HCD_RZ *hcd_rz, UX_RZ_ED *ed)
{
   ...
   if ( ed -> ux_rz_ed_endpoint -> ux_endpoint_descriptor.bDescriptorType == UX_CONTROL_ENDPOINT )

        /* This is a control enpoint.  */
        _ux_hcd_rz_control_td_add(hcd_rz,ed);
        
    else
    
           /* This is either a BULK or Interrupt endpoint.  */
        _ux_hcd_rz_bulk_int_td_add(hcd_rz,ed);
