...
void GUIX_touch_interrupt( void* nothing );
static void touch_interrupt_configure( void );
...
static void touch_interrupt_configure( void )
{
	...
	error = R_LCD_Open( TP_INT_PRI, 0, 0 );

	if ( 0 <= error )
	{

		error = R_LCD_EventEntry( LCDEVT_ENTRY_TP, GUIX_touch_interrupt );
