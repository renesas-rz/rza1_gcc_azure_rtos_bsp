...
* @brief        VDC driver parameter check processing
******************************************************************************/
...
/* Valid range for NFS */
#define VDC_CPARA_LVDSPLL_FD_LIMIT_56 (56u)
#define VDC_CPARA_LVDSPLL_FD_LIMIT_24 (24u)
#define VDC_CPARA_LVDSPLL_FD_LIMIT_28 (28u)
#define VDC_CPARA_LVDSPLL_FD_LIMIT_31 (31u)
#define VDC_CPARA_LVDSPLL_FD_LIMIT_37 (37u)
#define VDC_CPARA_LVDSPLL_FD_LIMIT_39 (39u)
#define VDC_CPARA_LVDSPLL_FD_LIMIT_46 (46u)
#define VDC_CPARA_LVDSPLL_FD_LIMIT_47 (47u)
#define VDC_CPARA_LVDSPLL_FD_LIMIT_55 (55u)
...
vdc_error_t VDC_InitializeCheckPrm (const vdc_channel_t ch, const vdc_init_t * const param)
{
    ...
    rsrc_state = VDC_ShrdPrmGetResource(ch, VDC_RESOURCE_VIDEO_IN);
    if (rsrc_state != VDC_RESOURCE_ST_INVALID)
    {
        inp_sel_tmp = VDC_ShrdPrmGetInputSelect(ch);
        if (((inp_sel_tmp == VDC_INPUT_SEL_VDEC) && (param->panel_icksel == VDC_PANEL_ICKSEL_IMG_DV)) ||
            ((inp_sel_tmp == VDC_INPUT_SEL_EXT) && (param->panel_icksel == VDC_PANEL_ICKSEL_IMG)))
        {
    ...
    else
    {
        ...
        if ((lvds_tmp->lvds_idiv_set >= VDC_LVDS_NDIV_NUM) || (lvds_tmp->lvds_odiv_set >= VDC_LVDS_NDIV_NUM))
        {
            error = VDC_ERR_PARAM_UNDEFINED;
            goto END;
        }
        /* Internal parameter setting for LVDS PLL */
        if (((uint32_t)lvds_tmp->lvdspll_tst & (uint32_t)~VDC_CPARA_RANGE_0X0000003F) != 0u)
        {
        ...
        if (((uint32_t)lvds_tmp->lvdspll_fd & (uint32_t)~VDC_CPARA_RANGE_0X000007FF) != 0u)
        {
            error = VDC_ERR_PARAM_BIT_WIDTH;
            goto END;
        }
        if ((uint32_t)lvds_tmp->lvdspll_fd < (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_56)
        {
            if ((uint32_t)lvds_tmp->lvdspll_fd < (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_24)
            {
                error = VDC_ERR_PARAM_EXCEED_RANGE;
                goto END;
            }
            if (((uint32_t)lvds_tmp->lvdspll_fd >= (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_28) && ((uint32_t)lvds_tmp->lvdspll_fd <= (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_31))
            {
                error = VDC_ERR_PARAM_EXCEED_RANGE;
                goto END;
            }
            if (((uint32_t)lvds_tmp->lvdspll_fd >= (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_37) && ((uint32_t)lvds_tmp->lvdspll_fd <= (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_39))
            {
                error = VDC_ERR_PARAM_EXCEED_RANGE;
                goto END;
            }
            if (((uint32_t)lvds_tmp->lvdspll_fd == (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_46) ||
                ((uint32_t)lvds_tmp->lvdspll_fd == (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_47) ||
                ((uint32_t)lvds_tmp->lvdspll_fd == (uint32_t)VDC_CPARA_LVDSPLL_FD_LIMIT_55))
            {
                error = VDC_ERR_PARAM_EXCEED_RANGE;
                goto END;
            }
        ...
        if (((uint32_t)lvds_tmp->lvdspll_rd & (uint32_t)~VDC_CPARA_RANGE_0X0000001F) != 0u)
        {
            error = VDC_ERR_PARAM_BIT_WIDTH;
            goto END;
        }
        /* The frequency dividing value (NOD) for the output frequency */
        if (lvds_tmp->lvdspll_od >= VDC_LVDS_PLL_NOD_NUM)
        {
            error = VDC_ERR_PARAM_UNDEFINED;
            goto END;
        }
...
vdc_error_t VDC_VideoInputCheckPrm (const vdc_channel_t ch, const vdc_input_t * const param)
{
    ...
    vdc_panel_clksel_t         panel_clksel;
    vdc_resource_state_t       rsrc_state;
    ...
    rsrc_state = VDC_ShrdPrmGetResource(ch, VDC_RESOURCE_PANEL_CLK);
    if (rsrc_state != VDC_RESOURCE_ST_INVALID)
    {
        panel_clksel = VDC_ShrdPrmGetPanelClkSel(ch);
        if (param->inp_sel == VDC_INPUT_SEL_VDEC)
        {   /* Video decoder output signals */
            if (panel_clksel == VDC_PANEL_ICKSEL_IMG_DV)
        ...
        {   /* Signals supplied via the external input pins */
            if (panel_clksel == VDC_PANEL_ICKSEL_IMG)
...
vdc_error_t VDC_CallbackISRCheckPrm (const vdc_channel_t ch, const vdc_int_t * const param)
{
    ...
    if (param->callback != 0)
    {
        if ((param->type == VDC_INT_TYPE_VLINE) ||
            (param->type == VDC_INT_TYPE_S0_WLINE) ||
            (param->type == VDC_INT_TYPE_S1_WLINE) ||
            (param->type == VDC_INT_TYPE_OIR_VLINE))
        {
            /* Line interrupt set */
    ...
    const vdc_write_t  * const param)
{
    ...
    if ((layer_id != VDC_LAYER_ID_0_WR) && (layer_id != VDC_LAYER_ID_1_WR) && (layer_id != VDC_LAYER_ID_OIR_WR))
    {
    ...
    const vdc_read_t   * const param)
{
    ...
    switch (graphics_id)
    {
        case VDC_GR_TYPE_GR0:
            btm_base_tmp    = VDC_ShrdPrmGetFrBuffBtm(ch, VDC_SC_TYPE_SC0);
            rsrc_state      = VDC_ShrdPrmGetLayerResource(ch, VDC_LAYER_ID_0_WR);
        break;
        case VDC_GR_TYPE_GR1:
            if (param->gr_flm_sel == VDC_GR_FLM_SEL_DISTORTION)
            {   /* IMR-LS2 is implemented in scaler 0 only. */
                error = VDC_ERR_PARAM_CONDITION;
                goto END;
            }
            btm_base_tmp    = VDC_ShrdPrmGetFrBuffBtm(ch, VDC_SC_TYPE_SC1);
            rsrc_state      = VDC_ShrdPrmGetLayerResource(ch, VDC_LAYER_ID_1_WR);
        break;
        ...
        break;
        case VDC_GR_TYPE_OIR:
            if (param->gr_flm_sel == VDC_GR_FLM_SEL_POINTER_BUFF)
            {
                error = VDC_ERR_PARAM_CONDITION;
                goto END;
            }
            if ((ch != VDC_CHANNEL_0) && (param->gr_flm_sel == VDC_GR_FLM_SEL_DISTORTION))
    ...
    const vdc_period_rect_t    * const enh_area)
{
    ...
    if (sharp_param != NULL)
    {
        ...
        if (((uint32_t)sharp_param->hrz_sharp[VDC_IMGENH_SHARP_H1].shp_core & (uint32_t)~VDC_CPARA_RANGE_0X0000007F) != 0u)
        {
            error = VDC_ERR_PARAM_BIT_WIDTH;
            goto END;
        }
        if (((uint32_t)sharp_param->hrz_sharp[VDC_IMGENH_SHARP_H2].shp_core & (uint32_t)~VDC_CPARA_RANGE_0X0000007F) != 0u)
        {
            error = VDC_ERR_PARAM_BIT_WIDTH;
            goto END;
        }
        if (((uint32_t)sharp_param->hrz_sharp[VDC_IMGENH_SHARP_H3].shp_core & (uint32_t)~VDC_CPARA_RANGE_0X0000007F) != 0u)
        {
            error = VDC_ERR_PARAM_BIT_WIDTH;
            goto END;
        }
    ...
    const vdc_alpha_blending_t * const param)
{
    ...
    if ((layer_id != VDC_LAYER_ID_1_RD) && (layer_id != VDC_LAYER_ID_2_RD) && (layer_id != VDC_LAYER_ID_3_RD))
    {
        error = VDC_ERR_PARAM_LAYER_ID;
        goto END;
    }
    ...
    const vdc_alpha_blending_rect_t    * const param)
{
    ...
    if ((layer_id != VDC_LAYER_ID_1_RD) &&
        ...
        (layer_id != VDC_LAYER_ID_3_RD) &&
        (layer_id != VDC_LAYER_ID_VIN_RD))
    {
        error = VDC_ERR_PARAM_LAYER_ID;
        goto END;
    }
    ...
    if (param != NULL)
    {
        ...
        if (param->alpha_rect != NULL)
        {
            ...
            if ((param->alpha_rect->gr_arc_coef < (int16_t)VDC_CPARA_GR_ARC_COEF_MIN) || (param->alpha_rect->gr_arc_coef > (int16_t)VDC_CPARA_GR_ARC_COEF_MAX))
            {
                error = VDC_ERR_PARAM_EXCEED_RANGE;
                goto END;
            }
...
static vdc_error_t CheckLayerId (const vdc_layer_id_t layer_id)
{
    ...
    if (layer_id == VDC_LAYER_ID_VIN_RD)
    {
        error = VDC_ERR_PARAM_LAYER_ID;
        goto END;
    }
...
static vdc_error_t CheckGraphicsTypeLayerId (const vdc_layer_id_t layer_id)
{
    ...
    if ((layer_id < VDC_LAYER_ID_0_RD) || (layer_id > VDC_LAYER_ID_OIR_RD))
    {
        error = VDC_ERR_PARAM_LAYER_ID;
        goto END;
    }
