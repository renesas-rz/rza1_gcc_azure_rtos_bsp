...
* @brief        VDC driver register setup processing
******************************************************************************/
...
#define VDC_REG_MASK_0X0703FF02    (0x0703FF02u) /* mask regaddr_lvds->lclkselr             */
#define VDC_REG_MASK_0X07FF1F30    (0x07FF1F30u) /* mask regaddr_lvds->lpllsetr             */
...
static void SetInitialConnection(const vdc_channel_t ch);
static void SetGrapicsBackgroundColor(const vdc_channel_t ch);
static void SetScalerBackgroundColor(
    const vdc_channel_t        ch,
    const vdc_graphics_type_t  graphics_id,
    const vdc_color_space_t    color_space);
...
static void SetInputCntrlColorMtx(const vdc_channel_t ch, const vdc_scaling_type_t scaling_id);
...
static void SetRwProcAbility_Write_0(const vdc_channel_t ch, const vdc_onoff_t ability);
static void SetRwProcAbility_Write_1(const vdc_channel_t ch, const vdc_onoff_t ability);
static void SetRwProcAbility_Write_OIR(const vdc_channel_t ch, const vdc_onoff_t ability);
static void SetRwProcAbility_Read_0(const vdc_channel_t ch, const vdc_onoff_t ability);
static void SetRwProcAbility_Read_1(const vdc_channel_t ch, const vdc_onoff_t ability);
static void SetRwProcAbility_Read_2(const vdc_channel_t ch, const vdc_onoff_t ability);
static void SetRwProcAbility_Read_3(const vdc_channel_t ch, const vdc_onoff_t ability);
static void SetRwProcAbility_Read_OIR(const vdc_channel_t ch, const vdc_onoff_t ability);
static void SetRegUpdateRwEnable(const vdc_channel_t ch, const vdc_layer_id_t layer_id);

static void SetGraphicsDisplayMode(
    const vdc_channel_t                ch,
    const vdc_layer_id_t               layer_id,
    const vdc_gr_disp_sel_t    * const gr_disp_sel);
...
static void SetUndSel(const vdc_channel_t ch, const vdc_onoff_t und_sel);
...
static void (* const rw_proc_function_tbl[VDC_LAYER_ID_NUM])(const vdc_channel_t ch, const vdc_onoff_t ability) =
{
 ...
 * @retval      None
 *****************************************************************************/
void VDC_Initialize (const vdc_channel_t ch, const vdc_init_t * const param)
{
    ...
    {   /* Panel clock frequency division ratio */
        (uint16_t)VDC_REG_CLKDIV_1_1, (uint16_t)VDC_REG_CLKDIV_1_2,  (uint16_t)VDC_REG_CLKDIV_1_3,  (uint16_t)VDC_REG_CLKDIV_1_4,
        (uint16_t)VDC_REG_CLKDIV_1_5, (uint16_t)VDC_REG_CLKDIV_1_6,  (uint16_t)VDC_REG_CLKDIV_1_7,  (uint16_t)VDC_REG_CLKDIV_1_8,
        (uint16_t)VDC_REG_CLKDIV_1_9, (uint16_t)VDC_REG_CLKDIV_1_12, (uint16_t)VDC_REG_CLKDIV_1_16, (uint16_t)VDC_REG_CLKDIV_1_24,
        (uint16_t)VDC_REG_CLKDIV_1_32
    };
    static const uint16_t       panel_icksel_tbl[VDC_PANEL_ICKSEL_NUM] =
    {   /* Panel clock select */
        (uint16_t)VDC_REG_ICKSEL_IMG,  (uint16_t)VDC_REG_ICKSEL_IMG_DV, (uint16_t)VDC_REG_ICKSEL_EXT_0, (uint16_t)VDC_REG_ICKSEL_EXT_1,
        (uint16_t)VDC_REG_ICKSEL_PERI, (uint16_t)VDC_REG_ICKSEL_LVDS,   (uint16_t)VDC_REG_ICKSEL_LVDS_DIV7
    };

    syscnt_panel_clk_reg = vdc_regaddr_system_ctrl[ch].syscnt_panel_clk;
    /* Disable */
    ...
    *syscnt_panel_clk_reg   = syscnt_panel_clk_tmp;
    ...
    if (lvds_tmp != NULL)
    {
        ...
        *(regaddr_lvds->lpllsetr) |= (uint32_t)VDC_REG_BIT0;
        ...
        /* LPHYACC */
        *(regaddr_lvds->lphyacc) &= (uint32_t)~VDC_REG_MASK_0X00000003;
        *(regaddr_lvds->lphyacc) |= LVDS_LPHYACC_VALUE;
        /* LCLKSELR: LVDS clock select register */
        *(regaddr_lvds->lclkselr) &= (uint32_t)~VDC_REG_MASK_0X0703FF02;
        ...
        *(regaddr_lvds->lclkselr) |= (uint32_t)lvds_tmp->lvds_idiv_set << VDC_REG_SHIFT_16;
            /* Internal parameter setting for LVDS PLL */
        *(regaddr_lvds->lclkselr) |= (uint32_t)lvds_tmp->lvdspll_tst << VDC_REG_SHIFT_10;
        ...
        *(regaddr_lvds->lclkselr) |= (uint32_t)lvds_tmp->lvds_odiv_set << VDC_REG_SHIFT_8;
        if (lvds_tmp->lvds_vdc_sel != VDC_CHANNEL_0)
        {
            /* A channel in VDC whose data is to be output through the LVDS */
            *(regaddr_lvds->lclkselr) |= (uint32_t)VDC_REG_BIT1;
        }

        /* LPLLSETR: LVDS PLL setting register */
        *(regaddr_lvds->lpllsetr) &= (uint32_t)~VDC_REG_MASK_0X07FF1F30;
        ...
        *(regaddr_lvds->lpllsetr) |= (uint32_t)lvds_tmp->lvdspll_od << VDC_REG_SHIFT_4;
    ...
    inp_sel_cnt_reg = vdc_regaddr_input_ctrl[ch].inp_sel_cnt;
    /* Input select */
 ...
 * @retval      None
******************************************************************************/
void VDC_Terminate (const vdc_channel_t ch)
{
    volatile uint16_t         * syscnt_panel_clk_reg;
    uint16_t                    syscnt_panel_clk_tmp;
    ...
    *syscnt_panel_clk_reg   = syscnt_panel_clk_tmp;
    ...
    rsrc_state = VDC_ShrdPrmGetResource(ch, VDC_RESOURCE_LVDS_CLK);
 ...
 * @retval      None
******************************************************************************/
void VDC_VideoInput (const vdc_channel_t ch, const vdc_input_t * const param)
{
    const vdc_regaddr_input_ctrl_t   * input_ctrl;
    const vdc_sync_delay_t           * dly_tmp;
    const vdc_ext_in_sig_t           * ext_sig_tmp;

    input_ctrl = &vdc_regaddr_input_ctrl[ch];
 ...
 * @retval      None
******************************************************************************/
void VDC_SyncControl (const vdc_channel_t ch, const vdc_sync_ctrl_t * const param)
{
    const vdc_regaddr_scaler_t   * scaler_0;
    const vdc_regaddr_scaler_t   * scaler_1;
    const vdc_regaddr_scaler_t   * scaler_oir;
    uint32_t                        full_scr;

    scaler_0    = &vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC0];
    scaler_1    = &vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC1];
    scaler_oir  = &vdc_regaddr_scaler[ch][VDC_SC_TYPE_OIR];
    ...
    {   /* Frequent Vsync signal masking and missing Vsync signal compensation are disabled, if unnecessary. */
        ...
        *(scaler_1->scl0_frc2) &= (uint32_t)~VDC_REG_BIT0;
        *(scaler_1->scl0_frc2) |= (uint32_t)VDC_REG_BIT_0XFFFF0000;
    }
    /* In OIR, frequent Vsync signal masking and missing Vsync signal compensation are set to off. */
    *(scaler_oir->scl0_frc1) &= (uint32_t)~VDC_REG_BIT0;
    *(scaler_oir->scl0_frc2) &= (uint32_t)~VDC_REG_BIT0;
    ...
    *(scaler_oir->scl0_frc4)    = (uint32_t)param->res_fh;
    ...
    if (param->res_vs_sel == VDC_OFF)
    {
        ...
        *(scaler_1->scl0_frc3) &= (uint32_t)~VDC_REG_BIT0;
    }
    else
    {
        *(scaler_0->scl0_frc3) |= (uint32_t)VDC_REG_BIT0;
        ...
        *(scaler_1->scl0_frc4) &= (uint32_t)~VDC_REG_MASK_0X07FF0000;
        *(scaler_1->scl0_frc4) |= (uint32_t)param->res_fv << VDC_REG_SHIFT_16;
    }
    /* In OIR, external input Vsync signal is selected automatically. */
    *(scaler_oir->scl0_frc3) &= (uint32_t)~VDC_REG_BIT0;

    /* Sync signal output and full-screen enable signal select: Scaler 0 */
    *(scaler_0->scl0_frc3) &= (uint32_t)~VDC_REG_BIT8;
    *(scaler_1->scl0_frc3) |= (uint32_t)VDC_REG_BIT8;

    /* Vsync signal delay control */
    *(scaler_0->scl0_frc5) &= (uint32_t)~VDC_REG_MASK_0X000000FF;
    *(scaler_0->scl0_frc5) |= (uint32_t)param->res_vsdly;
    *(scaler_1->scl0_frc5) &= (uint32_t)~VDC_REG_MASK_0X000000FF;
    *(scaler_1->scl0_frc5) |= (uint32_t)param->res_vsdly;
    *(scaler_oir->scl0_frc5) &= (uint32_t)~VDC_REG_MASK_0X000000FF;
    ...
    *(scaler_1->scl0_update)   |= (uint32_t)(VDC_REG_BIT8|VDC_REG_BIT4);
    *(scaler_oir->scl0_update) |= (uint32_t)(VDC_REG_BIT8|VDC_REG_BIT4);

    SetInitialConnection(ch);

    VDC_ShrdPrmSetUndSel(ch, VDC_OFF);
 ...
 * @retval      None
******************************************************************************/
void VDC_DisplayOutput (const vdc_channel_t ch, const vdc_output_t * const param)
{
    ...
    output_ctrl = &vdc_regaddr_output_ctrl[ch];
    ...
    *(output_ctrl->out_pdtha) |= dither_format[param->out_format];
    ...
    SetGraphicsDisplayMode(ch, VDC_LAYER_ID_ALL, gr_disp_sel_tbl);
    ...
    const vdc_write_t  * const param)
{
    ...
    scaler      = &vdc_regaddr_scaler[ch][scaling_id];
    ...
    *(scaler->scl1_wr5) |= (uint32_t)param->res_fs_rate << VDC_REG_SHIFT_8;
    ...
    {   /* OIR */
        ...
        *(scaler->scl0_ds7)  = (uint32_t)scldw_rot->res.vw << VDC_REG_SHIFT_16;
        *(scaler->scl0_ds7) |= (uint32_t)scldw_rot->res.hw;

        /* Vertical scaling-down and horizontal scaling-down : OFF */
        *(scaler->scl0_ds1) &= (uint32_t)~VDC_REG_MASK_0X00000011;
    ...
    *(scaler->scl0_ds3) |= (uint32_t)scldw_rot->res.hw;
    ...
    {   /* Scaler 0 or 1 */
        ...
        *(scaler->scl0_ds7) |= (uint32_t)scldw_rot->res_out_hw;
        ...
        res_inter_tmp = VDC_ShrdPrmGetInterlace(ch, scaling_id);
        /* Scaling-down */
    ...
    const vdc_read_t   * const param)
{
    ...
    w_read_fb = VDC_ShrdPrmGetFrBuffWidth_Rd(ch, graphics_id);
    ...
    if ((graphics_id == VDC_GR_TYPE_GR0) ||
        (graphics_id == VDC_GR_TYPE_GR1) ||
        (graphics_id == VDC_GR_TYPE_OIR))
    {
        SetScalerGraphics(ch, graphics_id, w_read_fb, &param->gr_grc, param->gr_flm_sel, graphics_enlargement);
    }

    img_synthesizer = &vdc_regaddr_img_synthesizer[ch][graphics_id];
    ...
    *(img_synthesizer->gr_flm1) |= (uint32_t)param->gr_flm_sel << VDC_REG_SHIFT_8;
    ...
    /* Frame buffer base address */
    *(img_synthesizer->gr_flm2) = (uint32_t)param->gr_base & (uint32_t)~VDC_REG_MASK_0X00000007;
    ...
    *(img_synthesizer->gr_ab3) |= (uint32_t)param->gr_grc.hw;
    ...
    if ((graphics_id == VDC_GR_TYPE_GR1) ||
        (graphics_id == VDC_GR_TYPE_GR2) ||
        (graphics_id == VDC_GR_TYPE_GR3))
    {
    ...
    *(img_synthesizer->gr_ab7) &= (uint32_t)~VDC_REG_BIT0;
    ...
    if ((graphics_id == VDC_GR_TYPE_GR0) || (graphics_id == VDC_GR_TYPE_OIR))
    ...
    if ((graphics_id == VDC_GR_TYPE_GR0) || (graphics_id == VDC_GR_TYPE_GR1))
    {
        ...
        *(img_synthesizer->gr_flm6) &= (uint32_t)~VDC_REG_MASK_0X0000E000;
        ...
        color_space = VDC_ShrdPrmGetColorSpaceFbRd(ch, graphics_id);
        ...
        if (graphics_id == VDC_GR_TYPE_GR0)
        {
            img_qlty_imp = &vdc_regaddr_img_qlty_imp[ch][VDC_IMG_IMPRV_0];
        }
        else
        {
            img_qlty_imp = &vdc_regaddr_img_qlty_imp[ch][VDC_IMG_IMPRV_1];
        }
        /* Operating mode */
    ...
    rsrc_state = VDC_ShrdPrmGetLayerResource(ch, VDC_LAYER_ID_1_RD);
    if ((graphics_id == VDC_GR_TYPE_GR0) && (rsrc_state == VDC_RESOURCE_ST_INVALID))
    {   /* Graphics 0 is specified when graphics 1 is not used. */
        SetImgQaImproverColorMtx(ch, VDC_GR_TYPE_GR1, VDC_COLOR_SPACE_GBR);
    }
    ...
    const vdc_read_chg_t   * const param)
{
    ...
    gr_flm_sel_tmp  = VDC_ShrdPrmGetSelFbAddrSig(ch, graphics_id);
    gr_grc_tmp      = VDC_ShrdPrmGetDisplayArea(ch, graphics_id);
    w_read_fb       = VDC_ShrdPrmGetFrBuffWidth_Rd(ch, graphics_id);
    ...
    if (param->gr_base != NULL)
    {
        *(img_synthesizer->gr_flm2) = (uint32_t)param->gr_base & (uint32_t)~VDC_REG_MASK_0X00000007;
        ...
        {
            gr_format_tmp = VDC_ShrdPrmGetGraphicsFormat(ch, graphics_id);
            /* Sets the amount of data to be skipped through */
 ...
 * @retval      None
******************************************************************************/
void VDC_StartProcess (const vdc_channel_t ch, const vdc_layer_id_t layer_id, const vdc_start_t * const param)
{
    uint32_t                    layer_id_num;
    vdc_resource_state_t       resource_state;
    vdc_resource_state_t       oir_resrc_st_before;
    vdc_resource_state_t       oir_resrc_st_after;
    const vdc_regaddr_scaler_t * scaler;
    ...
    oir_resrc_st_before = VDC_ShrdPrmGetOirRwProcEnabled(ch);

    if (layer_id != VDC_LAYER_ID_ALL)
    {
        rw_proc_function_tbl[layer_id](ch, VDC_ON);
    }
    else
    {
        for (layer_id_num = 0; layer_id_num < (uint32_t)VDC_LAYER_ID_NUM; layer_id_num++)
        {
            resource_state = VDC_ShrdPrmGetRwProcReady(ch, (vdc_layer_id_t)layer_id_num);
            if (resource_state != VDC_RESOURCE_ST_INVALID)
            {
                rw_proc_function_tbl[layer_id_num](ch, VDC_ON);
            }
        }
    }
    SetRegUpdateRwEnable(ch, layer_id);

    oir_resrc_st_after = VDC_ShrdPrmGetOirRwProcEnabled(ch);

    /* OIR */
    if ((oir_resrc_st_before == VDC_RESOURCE_ST_INVALID) &&
        (oir_resrc_st_after != VDC_RESOURCE_ST_INVALID))
    {
        scaler = &vdc_regaddr_scaler[ch][VDC_SC_TYPE_OIR];
        *(scaler->scl0_frc3) |= (uint32_t)VDC_REG_BIT16;
        /* Register update control register (OIR_SCL0_UPDATE)
           b8       SCL0_UPDATE */
        *(scaler->scl0_update) |= (uint32_t)VDC_REG_BIT8;
    }
 ...
 * @retval      None
******************************************************************************/
void VDC_StopProcess (const vdc_channel_t ch, const vdc_layer_id_t layer_id)
{
    uint32_t                    layer_id_num;
    vdc_resource_state_t       resource_state;
    vdc_resource_state_t       oir_resrc_st_before;
    ...
    if (layer_id != VDC_LAYER_ID_ALL)
    {
        rw_proc_function_tbl[layer_id](ch, VDC_OFF);
    }
    else
    {
        for (layer_id_num = 0; layer_id_num < (uint32_t)VDC_LAYER_ID_NUM; layer_id_num++)
        {
            resource_state = VDC_ShrdPrmGetRwProcEnabled(ch, (vdc_layer_id_t)layer_id_num);
            if (resource_state != VDC_RESOURCE_ST_INVALID)
            {
                rw_proc_function_tbl[layer_id_num](ch, VDC_OFF);
            }
        }
    }
    SetRegUpdateRwEnable(ch, layer_id);
 ...
 * @retval      None
******************************************************************************/
void VDC_ReleaseDataControl (const vdc_channel_t ch, const vdc_layer_id_t layer_id)
{
    const vdc_regaddr_scaler_t           * scaler;
    vdc_resource_state_t                   resource_state;
    const vdc_regaddr_img_synthesizer_t  * img_synthesizer_vin;

    resource_state = VDC_ShrdPrmGetRwProcReady(ch, VDC_LAYER_ID_1_RD);
    if ((layer_id == VDC_LAYER_ID_1_RD) ||
        ((layer_id == VDC_LAYER_ID_ALL) && (resource_state!= VDC_RESOURCE_ST_INVALID)))
    {
        SetInitialConnection(ch);

        /* Use Vsync and enable signal output from scaler 0 */
        scaler = &vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC0];
        *(scaler->scl0_frc3) &= (uint32_t)~VDC_REG_BIT8;
        /* Register update control register (SC0_SCL0_UPDATE)
           b8       SCL0_UPDATE */
        *(scaler->scl0_update) |= (uint32_t)VDC_REG_BIT8;

        scaler = &vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC1];
        *(scaler->scl0_frc3) |= (uint32_t)VDC_REG_BIT8;
    ...
    resource_state = VDC_ShrdPrmGetRwProcReady(ch, VDC_LAYER_ID_0_RD);
    if ((layer_id == VDC_LAYER_ID_0_RD) ||
        ((layer_id == VDC_LAYER_ID_ALL) && (resource_state!= VDC_RESOURCE_ST_INVALID)))
    {
        img_synthesizer_vin = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_VIN];

        /* Turns off alpha blending in a rectangular area */
        *(img_synthesizer_vin->gr_ab1) &= (uint32_t)~VDC_REG_BIT12;
        /* Selection of lower-layer plane in scaler, graphics 1 is used as lower-layer graphics. */
        *(img_synthesizer_vin->gr_ab1) |= (uint32_t)VDC_REG_BIT2;
        /* Graphics display mode in VIN */
        *(img_synthesizer_vin->gr_ab1) &= (uint32_t)~VDC_REG_MASK_0X00000003;
        *(img_synthesizer_vin->gr_ab1) |= (uint32_t)VDC_DISPSEL_LOWER;
        /* Disable fade-in and fade-out */
        *(img_synthesizer_vin->gr_ab6) &= (uint32_t)~VDC_REG_MASK_0X01FF00FF;
        /* Set the initial alpha value to '255' */
        *(img_synthesizer_vin->gr_ab7) |= (uint32_t)VDC_REG_MASK_0X00FF0000;

        /* Graphics register update control register (GR_VIN_UPDATE)
           b8       GR_VIN_UPDATE
           b4       GR_VIN_P_VEN */
        *(img_synthesizer_vin->gr_update) |= (uint32_t)(VDC_REG_BIT8|VDC_REG_BIT4);
    }
}   /* End of function VDC_ReleaseDataControl() */
 ...
 * @retval      None
******************************************************************************/
void VDC_ImageColorMatrix (const vdc_channel_t ch, const vdc_color_matrix_t * const param)
{
    SetColorMatrix(param->mtx_mode, param->offset, param->gain, &vdc_regaddr_color_matrix[ch][param->module]);
    ...
    const vdc_period_rect_t    * const enh_area)
{
    ...
    img_qlty_imp = &vdc_regaddr_img_qlty_imp[ch][imgimprv_id];
    ...
    const vdc_alpha_blending_rect_t    * const param)
{
    ...
    if (param != NULL)
    {
        ...
        {
            if (graphics_id != VDC_GR_TYPE_VIN)
            {
    ...
    const vdc_chromakey_t  * const param)
{
    ...
    if (param != NULL)
    {
            ...
            (gr_format_tmp == VDC_GR_FORMAT_CLUT8))
        {
            ...
            *(img_synthesizer->gr_ab9)  = ColorConversionIntoAgbr(param->rep_color & VDC_REG_RGB_24BIT_MASK);
        }
        else
        {
 ...
 * @retval      None
******************************************************************************/
void VDC_CLUT (const vdc_channel_t ch, const vdc_graphics_type_t graphics_id, const vdc_clut_t * const param)
{
    vdc_gr_format_t                        gr_format_tmp;
    const vdc_regaddr_img_synthesizer_t  * img_synthesizer;
    ...
    {
        Set_Clut(param, vdc_regaddr_clut[ch][graphics_id]);
 ...
 * @retval      None
******************************************************************************/
void VDC_DisplayCalibration (const vdc_channel_t ch, const vdc_disp_calibration_t * const param)
{
...
static void SetInitialConnection (const vdc_channel_t ch)
{
    ...
    img_synthesizer = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR1];

    /* Cascade ON */
    *(img_synthesizer->gr_ab1) |= (uint32_t)VDC_REG_BIT28;
    /* Graphics register update control register (GR1_UPDATE)
       b8       GR1_UPDATE */
    *(img_synthesizer->gr_update) |= (uint32_t)VDC_REG_BIT8;

    VDC_ShrdPrmSetCascade(ch, VDC_ON);

    img_synthesizer = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_VIN];

    /* Turns off alpha blending in a rectangular area */
    *(img_synthesizer->gr_ab1) &= (uint32_t)~VDC_REG_BIT12;
    /* Selection of lower-layer plane in scaler, graphics 1 is used as lower-layer graphics. */
    *(img_synthesizer->gr_ab1) |= (uint32_t)VDC_REG_BIT2;
    /* Graphics display mode in VIN */
    *(img_synthesizer->gr_ab1) &= (uint32_t)~VDC_REG_MASK_0X00000003;
    *(img_synthesizer->gr_ab1) |= (uint32_t)VDC_DISPSEL_LOWER;
    /* Disable fade-in and fade-out */
    ...
    *(img_synthesizer->gr_ab7) |= (uint32_t)VDC_REG_MASK_0X00FF0000;
    /* Graphics register update control register (GR_VIN_UPDATE)
       b8       GR_VIN_UPDATE
       b4       GR_VIN_P_VEN */
    *(img_synthesizer->gr_update) |= (uint32_t)(VDC_REG_BIT8|VDC_REG_BIT4);
...
static void SetGrapicsBackgroundColor (const vdc_channel_t ch)
{
    ...
    vdc_graphics_type_t    gr_type_index;

    /* Background color in 24-bit RGB color format or CrYCb format */
    bg_color_tmp = VDC_ShrdPrmGetBgColor(ch, VDC_COLOR_SPACE_GBR);
    ...
    /* GR2, GR3, VIN, OIR */
    for (gr_type_index = VDC_GR_TYPE_GR2; gr_type_index < VDC_GR_TYPE_NUM; gr_type_index++)
    {
        /* Background color in GBR format (GRx_BASE) */
        bg_color_reg    = vdc_regaddr_img_synthesizer[ch][gr_type_index].gr_base;
        *bg_color_reg   = bg_color_conv;
        /* Graphics register update control register (GRx_UPDATE)
           b4       GRx_P_VEN */
        update_reg      = vdc_regaddr_img_synthesizer[ch][gr_type_index].gr_update;
        *update_reg    |= (uint32_t)VDC_REG_BIT4;
    }
    /* For OIR */
    /* Background color in RGB format (OIR_SCL0_OVR1) */
    bg_color_reg    = vdc_regaddr_scaler[ch][VDC_SC_TYPE_OIR].scl0_ovr1;
    *bg_color_reg   = bg_color_tmp;
    /* Register update control register (OIR_SCL0_UPDATE)
       b4       SCL0_VEN_B */
    update_reg      = vdc_regaddr_scaler[ch][VDC_SC_TYPE_OIR].scl0_update;
    *update_reg    |= (uint32_t)VDC_REG_BIT4;
    ...
    const vdc_color_space_t    color_space)
{
    ...
    if (graphics_id == VDC_GR_TYPE_GR0)
    {
        scl0_ovr1_reg   = vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC0].scl0_ovr1;
        scl0_update_reg = vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC0].scl0_update;
        gr_base_reg     = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR0].gr_base;
        gr_update_reg   = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR0].gr_update;
    }
    else
    {   /* graphics_id == VDC_GR_TYPE_GR1 */
        scl0_ovr1_reg   = vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC1].scl0_ovr1;
        scl0_update_reg = vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC1].scl0_update;
        gr_base_reg     = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR1].gr_base;
        gr_update_reg   = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR1].gr_update;
    }
    /* Background color in 24-bit RGB color format or CrYCb format */
    bg_color_tmp    = VDC_ShrdPrmGetBgColor(ch, color_space);
    /* Conversion from RGB into GBR / from CrYCb into YCbCr */
    bg_color_conv   = ColorConversionIntoAgbr(bg_color_tmp);
    bg_color_conv  &= (uint32_t)VDC_REG_RGB_24BIT_MASK;
...
static void SetInputCntrlColorMtx (const vdc_channel_t ch, const vdc_scaling_type_t scaling_id)
{
    ...
    if (scaling_id == VDC_SC_TYPE_SC0)
    {
        channel = ch;
    }
    else
    {
        channel = (ch == VDC_CHANNEL_0) ? VDC_CHANNEL_1 : VDC_CHANNEL_0;
    }
    color_space_input   = VDC_ShrdPrmGetColorSpace(channel);
    color_space_fb      = VDC_ShrdPrmGetColorSpaceFbWr(ch, scaling_id);
    ...
    SetColorMatrix(mtx_mode_tmp, offset_tmp, gain_tmp, &vdc_regaddr_color_matrix[channel][VDC_COLORMTX_IMGCNT]);
    ...
    const vdc_onoff_t                  gr_enlarge)
{
    ...
    if (graphics_id == VDC_GR_TYPE_GR0)
    {   /* Scaler 0 */
        scaling_id = VDC_SC_TYPE_SC0;
    }
    else if (graphics_id == VDC_GR_TYPE_GR1)
    {   /* Scaler 1 */
        scaling_id = VDC_SC_TYPE_SC1;
    }
    else
    {   /* OIR */
        scaling_id = VDC_SC_TYPE_OIR;
    }
    ...
    {   /* Video image or graphics enlargement */
        if (graphics_id == VDC_GR_TYPE_OIR)
        {   /* Turn the vertical scaling-up and the horizontal scaling-up off */
            *(scaler->scl0_us1) &= (uint32_t)~VDC_REG_MASK_0X00000011;
            /* Sync signals from the graphics processing block */
            *(scaler->scl0_us8) |= (uint32_t)VDC_REG_BIT4;
        }
        else
        {
            ...
            *(scaler->scl0_us4) |= (uint32_t)w_read_fb->in_hw;
            ...
            res_ds_wr_md_tmp    = VDC_ShrdPrmGetWritingMode(ch, scaling_id);
            adj_sel_tmp         = VDC_ShrdPrmGetMeasureFolding(ch, graphics_id);
            ...
            ln_off_dir = VDC_ShrdPrmGetLineOfsAddrDir(ch, graphics_id);
            /* Scaling-up */
    ...
    const vdc_onoff_t          gr_enlarge)
{
    ...
    const vdc_regaddr_scaler_t           * scaler_0;
    const vdc_regaddr_scaler_t           * scaler_1;
    vdc_res_vs_in_sel_t                    res_vs_in_sel_tmp;
    vdc_onoff_t                            cascade;

    cascade = VDC_ShrdPrmGetCascade(ch);
    /* Cascade */
    if (graphics_id == VDC_GR_TYPE_GR1)
    {
        img_synthesizer = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR1];
        ...
        {   /* Video image or graphics enlargement */
            ...
            *(img_synthesizer->gr_ab1) &= (uint32_t)~VDC_REG_BIT28;
            ...
            res_vs_in_sel_tmp = VDC_ShrdPrmGetVsInSel(ch);
            if (res_vs_in_sel_tmp == VDC_RES_VS_IN_SEL_SC0)
            {   /* Use Vsync and enable signal output from scaler 0 */
                *(scaler_0->scl0_frc3) &= (uint32_t)~VDC_REG_BIT8;
                *(scaler_1->scl0_frc3) |= (uint32_t)VDC_REG_BIT8;
            }
        ...
        VDC_ShrdPrmSetCascade(ch, cascade);

        /* Register update control register (SC0_SCL0_UPDATE)
           b8       SCL0_UPDATE */
        *(scaler_0->scl0_update) |= (uint32_t)VDC_REG_BIT8;
        /* Register update control register (SC1_SCL0_UPDATE)
           b8       SCL0_UPDATE */
        *(scaler_1->scl0_update) |= (uint32_t)VDC_REG_BIT8;
        /* Graphics register update control register (GR1_UPDATE)
           b8       GR1_UPDATE */
        *(img_synthesizer->gr_update) |= (uint32_t)VDC_REG_BIT8;
    }
    ...
    const vdc_onoff_t          gr_enlarge)
{
    ...
    rsrc_state  = VDC_ShrdPrmGetLayerResource(ch, VDC_LAYER_ID_0_RD);
    if ((cascade == VDC_OFF) &&
        ((rsrc_state != VDC_RESOURCE_ST_INVALID) || (graphics_id == VDC_GR_TYPE_GR0)))
    {   /* Cascade connection OFF and graphics 0 is used */
        img_synthesizer_vin = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_VIN];

        und_sel = VDC_ShrdPrmGetUndSel(ch);
        if (und_sel == VDC_OFF)
        ...
        {   /* Graphics 0 is allocated to the upper-layer. */
            *(img_synthesizer_vin->gr_ab1) |= (uint32_t)VDC_REG_BIT2;
            ...
            img_synthesizer = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR0];
        }
        /* Graphics display mode in VIN */
        *(img_synthesizer_vin->gr_ab1) &= (uint32_t)~VDC_REG_MASK_0X00000003;
        *(img_synthesizer_vin->gr_ab1) |= (uint32_t)VDC_DISPSEL_BLEND;
    ...
    const vdc_onoff_t          gr_enlarge)
{
    ...
    const vdc_regaddr_img_synthesizer_t  * img_synthesizer_vin;
    vdc_onoff_t                            cascade;
    vdc_onoff_t                            cascade_prev;
    vdc_resource_state_t                   rsrc_state;
    vdc_onoff_t                            und_sel;
    uint32_t                                reg_data;

    /* Cascade */
    cascade_prev    = VDC_ShrdPrmGetCascade(ch);
    cascade         = SetCascade(ch, graphics_id, gr_flm_sel, gr_enlarge);
...
static void SetImgQaImproverColorMtx (
    ...
    const vdc_color_space_t    color_space)
{
    vdc_colormtx_module_t  module_tmp;
    vdc_colormtx_mode_t    mtx_mode_tmp;
    const uint16_t        * offset_tmp;
    const uint16_t        * gain_tmp;
    ...
    /* Color matrix module */
    module_tmp = (graphics_id == VDC_GR_TYPE_GR0) ? VDC_COLORMTX_ADJ_0 : VDC_COLORMTX_ADJ_1;
    /* Color matrix offset (DC) adjustment (YG, B, and R) */
    offset_tmp = colormtx_offset_adj;
    /* Color matrix signal gain adjustment (GG, GB, GR, BG, BB, BR, RG, RB, and RR) */
    gain_tmp = colormtx_gain_adj[mtx_mode_tmp];

    SetColorMatrix(mtx_mode_tmp, offset_tmp, gain_tmp, &vdc_regaddr_color_matrix[ch][module_tmp]);
...
static void SetRwProcAbility_Write_0 (const vdc_channel_t ch, const vdc_onoff_t ability)
{
    ...
    {   /* SC0 frame buffer writing is disabled */
        *(scaler->scl1_wr5) &= (uint32_t)~VDC_REG_BIT0;
        ...
        VDC_ShrdPrmSetRwProcDisable(ch, VDC_LAYER_ID_0_WR);
    }
    else
    {   /* SC0 frame buffer writing is enabled */
        ...
        *(scaler->scl1_pbuf_cnt) &= (uint32_t)~VDC_REG_BIT16;
        ...
        VDC_ShrdPrmSetRwProcEnable(ch, VDC_LAYER_ID_0_WR);
    }
...
static void SetRwProcAbility_Write_1 (const vdc_channel_t ch, const vdc_onoff_t ability)
{
    ...
    {   /* SC1 frame buffer writing is disabled */
        *(scaler->scl1_wr5) &= (uint32_t)~VDC_REG_BIT0;
        ...
        VDC_ShrdPrmSetRwProcDisable(ch, VDC_LAYER_ID_1_WR);
    }
    else
    {   /* SC1 frame buffer writing is enabled */
        ...
        *(scaler->scl1_pbuf_cnt) &= (uint32_t)~VDC_REG_BIT16;
        ...
        VDC_ShrdPrmSetRwProcEnable(ch, VDC_LAYER_ID_1_WR);
    }
...
static void SetRwProcAbility_Write_OIR (const vdc_channel_t ch, const vdc_onoff_t ability)
{
    ...
    {   /* OIR frame buffer writing is disabled */
        *(scaler->scl1_wr5) &= (uint32_t)~VDC_REG_BIT0;
        ...
        VDC_ShrdPrmSetRwProcDisable(ch, VDC_LAYER_ID_OIR_WR);
    }
    else
    {   /* OIR frame buffer writing is enabled */
        *(scaler->scl1_wr5) |= (uint32_t)VDC_REG_BIT0;

        VDC_ShrdPrmSetRwProcEnable(ch, VDC_LAYER_ID_OIR_WR);
    }
...
static void SetRwProcAbility_Read_0 (const vdc_channel_t ch, const vdc_onoff_t ability)
{
    ...
    {   /* GR0 frame buffer reading is disabled */
        ...
        *(img_synthesizer->gr_ab1) |= (uint32_t)VDC_DISPSEL_BACK;
        ...
        VDC_ShrdPrmSetRwProcDisable(ch, VDC_LAYER_ID_0_RD);
    }
    else
    {   /* GR0 frame buffer reading is enabled */
        *(img_synthesizer->gr_flm_rd) |= (uint32_t)VDC_REG_BIT0;

        VDC_ShrdPrmSetRwProcEnable(ch, VDC_LAYER_ID_0_RD);
    }
...
static void SetRwProcAbility_Read_1 (const vdc_channel_t ch, const vdc_onoff_t ability)
{
    ...
    {   /* GR1 frame buffer reading is disabled */
        ...
        *(img_synthesizer->gr_ab1) |= (uint32_t)VDC_DISPSEL_LOWER;
        ...
        VDC_ShrdPrmSetRwProcDisable(ch, VDC_LAYER_ID_1_RD);
    }
    else
    {   /* GR1 frame buffer reading is enabled */
        *(img_synthesizer->gr_flm_rd) |= (uint32_t)VDC_REG_BIT0;

        VDC_ShrdPrmSetRwProcEnable(ch, VDC_LAYER_ID_1_RD);
    }
...
static void SetRwProcAbility_Read_2 (const vdc_channel_t ch, const vdc_onoff_t ability)
{
    ...
    /* GR2 reading */
    img_synthesizer = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR2];
    ...
    {   /* GR2 frame buffer reading is disabled */
        ...
        *(img_synthesizer->gr_ab1) |= (uint32_t)VDC_DISPSEL_LOWER;
        ...
        VDC_ShrdPrmSetRwProcDisable(ch, VDC_LAYER_ID_2_RD);
    }
    else
    {   /* GR2 frame buffer reading is enabled */
        *(img_synthesizer->gr_flm_rd) |= (uint32_t)VDC_REG_BIT0;

        VDC_ShrdPrmSetRwProcEnable(ch, VDC_LAYER_ID_2_RD);
    }
...
static void SetRwProcAbility_Read_3 (const vdc_channel_t ch, const vdc_onoff_t ability)
{
    ...
    /* GR3 reading */
    img_synthesizer = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR3];
    ...
    {   /* GR3 frame buffer reading is disabled */
        ...
        *(img_synthesizer->gr_ab1) |= (uint32_t)VDC_DISPSEL_LOWER;
        ...
        VDC_ShrdPrmSetRwProcDisable(ch, VDC_LAYER_ID_3_RD);
    }
    else
    {   /* GR3 frame buffer reading is enabled */
        *(img_synthesizer->gr_flm_rd) |= (uint32_t)VDC_REG_BIT0;

        VDC_ShrdPrmSetRwProcEnable(ch, VDC_LAYER_ID_3_RD);
    }
...
static void SetRwProcAbility_Read_OIR (const vdc_channel_t ch, const vdc_onoff_t ability)
{
    ...
    /* OIR reading */
    img_synthesizer = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_OIR];
    ...
    {   /* OIR frame buffer reading is disabled */
        ...
        *(img_synthesizer->gr_ab1) |= (uint32_t)VDC_DISPSEL_BACK;
        ...
        VDC_ShrdPrmSetRwProcDisable(ch, VDC_LAYER_ID_OIR_RD);
    }
    else
    {   /* OIR frame buffer reading is enabled */
        *(img_synthesizer->gr_flm_rd) |= (uint32_t)VDC_REG_BIT0;

        VDC_ShrdPrmSetRwProcEnable(ch, VDC_LAYER_ID_OIR_RD);
    }
...
static void SetRegUpdateRwEnable (const vdc_channel_t ch, const vdc_layer_id_t layer_id)
{
    ...
    switch (layer_id)
    {
        case VDC_LAYER_ID_0_WR:        /* Layer 0, write process */
            scl1_update_reg   = vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC0].scl1_update;
            *scl1_update_reg |= (uint32_t)(VDC_REG_BIT20 | VDC_REG_BIT4 | VDC_REG_BIT0);
        break;
        case VDC_LAYER_ID_1_WR:        /* Layer 1, write process */
            scl1_update_reg   = vdc_regaddr_scaler[ch][VDC_SC_TYPE_SC1].scl1_update;
            *scl1_update_reg |= (uint32_t)(VDC_REG_BIT20 | VDC_REG_BIT4 | VDC_REG_BIT0);
        break;
        case VDC_LAYER_ID_OIR_WR:      /* Layer OIR, write process */
            scl1_update_reg   = vdc_regaddr_scaler[ch][VDC_SC_TYPE_OIR].scl1_update;
            *scl1_update_reg |= (uint32_t)(VDC_REG_BIT4|VDC_REG_BIT0);
        break;
        case VDC_LAYER_ID_0_RD:        /* Layer 0, read process */
            gr_update_reg   = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR0].gr_update;
            *gr_update_reg |= (uint32_t)(VDC_REG_BIT4|VDC_REG_BIT0);
        break;
        case VDC_LAYER_ID_1_RD:        /* Layer 1, read process */
            gr_update_reg   = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR1].gr_update;
            *gr_update_reg |= (uint32_t)(VDC_REG_BIT4|VDC_REG_BIT0);
        break;
        case VDC_LAYER_ID_2_RD:        /* Layer 2, read process */
            gr_update_reg   = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR2].gr_update;
            *gr_update_reg |= (uint32_t)(VDC_REG_BIT4|VDC_REG_BIT0);
        break;
        case VDC_LAYER_ID_3_RD:        /* Layer 3, read process */
            gr_update_reg   = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_GR3].gr_update;
            *gr_update_reg |= (uint32_t)(VDC_REG_BIT4|VDC_REG_BIT0);
        break;
        case VDC_LAYER_ID_OIR_RD:      /* Layer OIR, read process */
            gr_update_reg   = vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_OIR].gr_update;
            *gr_update_reg |= (uint32_t)(VDC_REG_BIT4|VDC_REG_BIT0);
        break;
    ...
    const vdc_gr_disp_sel_t    * const gr_disp_sel)
{
    ...
    if (layer_id == VDC_LAYER_ID_ALL)
    {
        ...
        {
            if ((graphics_id != VDC_GR_TYPE_VIN) && (gr_disp_sel[graphics_id] != VDC_DISPSEL_IGNORED))
            {
                img_synthesizer = &vdc_regaddr_img_synthesizer[ch][graphics_id];
    ...
    else if ((layer_id >= VDC_LAYER_ID_0_RD) && (layer_id <= VDC_LAYER_ID_OIR_RD))
    {
...
static void SetUndSel (const vdc_channel_t ch, const vdc_onoff_t und_sel)
{
    const vdc_regaddr_img_synthesizer_t  * img_synthesizer;
    const vdc_regaddr_img_synthesizer_t  * img_synthesizer_vin;
    vdc_onoff_t                            current_und_sel;
    vdc_onoff_t                            cascade;
    vdc_resource_state_t                   rsrc_state;
    uint32_t                                reg_data;

    current_und_sel = VDC_ShrdPrmGetUndSel(ch);
    if (und_sel != current_und_sel)
    {
        VDC_ShrdPrmSetUndSel(ch, und_sel);

        cascade     = VDC_ShrdPrmGetCascade(ch);
        rsrc_state  = VDC_ShrdPrmGetLayerResource(ch, VDC_LAYER_ID_0_RD);
        if ((cascade == VDC_OFF) && (rsrc_state != VDC_RESOURCE_ST_INVALID))
        {   /* Cascade connection OFF and graphics 0 is used */
            img_synthesizer_vin = &vdc_regaddr_img_synthesizer[ch][VDC_GR_TYPE_VIN];
            if (und_sel == VDC_OFF)
            {   /* Graphics 1 is allocated to the upper-layer. */
...
static uint32_t ColorConversion (const vdc_gr_format_t gr_format, const uint32_t input_color)
{
    ...
    switch (gr_format)
    {
        ...
        case VDC_GR_FORMAT_RGBA8888:
            conv_color = ColorConversionIntoAgbr((uint32_t)((input_color >> VDC_REG_SHIFT_8) & VDC_REG_RGB_24BIT_MASK));
        break;
        default:
            /* DO NOTHING */
        break;
